...
public class MarketController extends HttpServlet {
	...
	//전체목록조회
    public String listCB(HttpServletRequest request, HttpServletResponse response) throws Exception {
        request.setAttribute("cashbooks", dao.getAll());
        int uid = 0;
       int total = dao.totalCB(uid);
       request.setAttribute("total", total);
        return "mainPage.jsp";
    }
    
    //총액
    public String totalCB(HttpServletRequest request, HttpServletResponse response) throws Exception { //우선 토탈값만
       int uid = 0;
       int total = dao.totalCB(uid);
       request.setAttribute("total", total);
       //request.getRequestDispatcher("/gagyebu/mainPage.jsp").forward(request, response);
       return "mainPage.jsp"; //String > void change
    }
}
